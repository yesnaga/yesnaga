...
describe('Board', () => {
	...
	describe('#getNeighbours', () => {
		it('works', () => {
			const neighbours = theBoard.getNeighbours(0, -1);
		...
		it('should give true for the players\'s start positions', () => {
			Object.values(theBoard.tokens).forEach((playerTokens) => {
				playerTokens.forEach((token) => {
					expect(theBoard.isOccupied(token.x, token.y)).toEqual(true);
				});
			});
		});
	...
	describe('#canMoveToken', () => {
		it('should give true for the players\'s start positions', () => {
			Object.values(theBoard.tokens).forEach((playerTokens) => {
				playerTokens.forEach((token) => {
					expect(theBoard.canMoveToken(token.x, token.y)).toEqual(true);
				});
			});
		});
	...
	describe('#canMoveTokenTo', () => {
		it('should give true for straight movement to empty tile without obstacle', () => {
			...
			expect(smallBoard.canMoveTokenTo(0, 0, 1, 0)).toEqual(true);
			...
			expect(lineBoard.canMoveTokenTo(1, 0, 2, 0)).toEqual(true);
		});
		...
		it('should give false if movement is not straight', () => {
			...
			expect(cornerBoard.canMoveTokenTo(0, -1, 1, 2)).toEqual(false);
		});
		...
		it('should give false if movement is blocked by own tokens', () => {
			...
			expect(lineBoard.canMoveTokenTo(0, 0, 2, 0)).toEqual(false);
		});
		...
		it('should enforce maximum movement length', () => {
			...
			expect(lineBoard.canMoveTokenTo(0, 0, 1, 0)).toEqual(false);
		});
		...
		it('should handle maximum movement length with obstacles', () => {
			...
			expect(lineBoard.canMoveTokenTo(0, 0, 1, 0)).toEqual(true);
		});
	});

	describe('#getMaximumTokenMovementInDirection', () => {
		it('should return the tile itself if no movement is possible', () => {
			expect(theBoard.getMaximumTokenMovementInDirection(0, 2, 'br')).toEqual(theBoard.discById[10]);
			expect(theBoard.getMaximumTokenMovementInDirection(0, -2, 'l')).toEqual(theBoard.discById[16]);
			expect(theBoard.getMaximumTokenMovementInDirection(-2, 0, 'tl')).toEqual(theBoard.discById[8]);
		});

		it('should return the correct tile after moving in the given direction', () => {
			expect(theBoard.getMaximumTokenMovementInDirection(0, 2, 'l')).toEqual(theBoard.discById[5]);
			expect(theBoard.getMaximumTokenMovementInDirection(0, -2, 'r')).toEqual(theBoard.discById[2]);
			expect(theBoard.getMaximumTokenMovementInDirection(0, 2, 'bl')).toEqual(theBoard.discById[11]);
			expect(theBoard.getMaximumTokenMovementInDirection(-2, 0, 'bl')).toEqual(theBoard.discById[4]);
		});
	});

	describe('#getMaximumTokenMovementTiles', () => {
		it('should return a list of possible movement tiles, filtered by viability', () => {
			expect(new Set(theBoard.getMaximumTokenMovementTiles(-1, 0))).toStrictEqual(new Set([
				theBoard.discById[4],
				theBoard.discById[7],
				theBoard.discById[9],
			...
			expect(new Set(theBoard.getMaximumTokenMovementTiles(-2, -1))).toStrictEqual(new Set([
				theBoard.discById[11],
				theBoard.discById[15],
			]));
			expect(new Set(theBoard.getMaximumTokenMovementTiles(0, 2))).toStrictEqual(new Set([
				theBoard.discById[5],
				theBoard.discById[9],
				theBoard.discById[11],
			]));
			expect(new Set(theBoard.getMaximumTokenMovementTiles(1, -1))).toStrictEqual(new Set([
				theBoard.discById[7],
				theBoard.discById[11],
			]));
		});
	...
	describe('#moveToken', () => {
		...
		it('should work for the default board', () => {
			theBoard.moveToken(-2, 0, 1, 0);
			theBoard.moveToken(0, 2, 0, -1);
			theBoard.moveToken(2, 2, -1, -1);
		...
		it('should work for custom boards', () => {
			...
			lineBoard.moveToken(0, 0, 1, 0);
			expect(lineBoard.tokens[0][0]).toStrictEqual({ x: 0, y: 0 });
			lineBoard.moveToken(0, 0, 2, 0);
