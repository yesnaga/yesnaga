...
describe('Board', () => {
	...
	describe('#isWon', () => {
		it('should give false for default board', () => {
			expect(theBoard.isWon()).toEqual(false);
		});

		it('should return true if three player tokens are connected', () => {
			const lineBoard = new Board([
				{ id: 0, x: 0, y: 0 },
				{ id: 1, x: 1, y: 0 },
				{ id: 2, x: 2, y: 0 },
			...
			expect(lineBoard.isWon()).toEqual(true);

			const hokeystickConditionBoard = new Board(undefined /* default tiles */, {
				0: [{ x: -1, y: -1 },
					{ x: 0, y: 0 },
					{ x: 0, y: 1 }],
				1: [{ x: -2, y: -2 },
			...
			expect(hokeystickConditionBoard.isWon()).toEqual(true);

			const triangleConditionBoard = new Board(undefined /* default tiles */, {
				0: [{ x: -1, y: -1 },
					{ x: 0, y: 0 },
					{ x: -1, y: 0 }],
				1: [{ x: -2, y: -2 },
			...
			expect(triangleConditionBoard.isWon()).toEqual(true);
		});
	});
});
