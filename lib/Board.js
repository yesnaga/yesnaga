...
class Board {
	...
	/**
	 * Returns whether a board is won, based on the token positions.
	 * If all tokens of a player border at least one token of the same player,
	 * they are positioned in one of the win conditions.
	 */
	isWon() {
		for (const [playerId, playerTokens] of Object.entries(this.tokens)) {
			if (playerTokens.every(
				(token) => this.getNeighbours(token).some(
					(neighbour) => this.tokenBelongsToPlayer(neighbour, playerId),
				),
			)) {
				return true;
			}
