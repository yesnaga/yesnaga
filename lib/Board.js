...
class Board {
	...
	moveDisc(from, to) {
		// remove disc from old position
		const originalNeighbours = this.getNeighbours(from);
		allDirections.forEach((direction) => {
			const thisNeighbour = originalNeighbours[direction];
			if (thisNeighbour) {
				delete this.tiles[thisNeighbour].neighbours[getOppositeDirection(direction)];
			}
		});

		// insert into new position and update reference neighbour
		const addDirection = allDirections.filter((d) => to[d])[0];
		const oppositeAddDirection = getOppositeDirection(addDirection);
		const newNeighbour = to[addDirection];
		this.tiles[from].neighbours[addDirection] = newNeighbour;
		this.tiles[newNeighbour].neighbours[oppositeAddDirection] = from;

		// update other discs
		const rightNeighbourDirection = getRightNeighbour(oppositeAddDirection);
		const rn = this.tiles[newNeighbour].neighbours[rightNeighbourDirection];
		if (rn) {
			this.tiles[rn].neighbours[getRightNeighbour(getOppositeDirection(rightNeighbourDirection))] = from;
		}

		const leftNeighbourDirection = getLeftNeighbour(oppositeAddDirection);
		const ln = this.tiles[newNeighbour].neighbours[leftNeighbourDirection];
		if (ln) {
			this.tiles[ln].neighbours[getLeftNeighbour(getOppositeDirection(leftNeighbourDirection))] = from;
		}
	}
}
