...
describe('Game', () => {
	...
	describe('use cases', () => {
		...
		describe('movement', () => {
			...
			it('allows to win the game via token movement', () => {
				...
				game.moveDisc({ from: { x: -2, y: -2 }, to: { x: -2, y: 0 } });
				expect(game.state.winner).toBe(undefined);

				game.moveToken({ from: { x: 2, y: 2 }, to: { x: 2, y: 1 } });
				game.moveDisc({ from: { x: 2, y: 2 }, to: { x: -2, y: -2 } });
				expect(game.state.winner).toBe(undefined);

				game.moveToken({ from: { x: 0, y: 2 }, to: { x: -2, y: 0 } });
				game.moveDisc({ from: { x: 0, y: 2 }, to: { x: 2, y: 2 } });
				expect(game.state.winner).toBe(undefined);
